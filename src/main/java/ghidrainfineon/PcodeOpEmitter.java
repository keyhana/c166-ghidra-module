...
public class PcodeOpEmitter {
	...
	/**
	 * Appends the pcode to assign a register to another register
	 * @param src
	 * @param dst
	 */
	public void emitAssignRegisterToRegister(Varnode src, String dst) {
		Varnode dstVn = findRegister(dst);
		Varnode[] in = new Varnode[1];
		in[0] = src;
		PcodeOp op = new PcodeOp(opAddress, seqnum++, PcodeOp.COPY, in, dstVn);
		opList.add(op);
	}
